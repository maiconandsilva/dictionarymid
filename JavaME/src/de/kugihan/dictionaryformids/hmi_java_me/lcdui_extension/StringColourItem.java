...
public  class StringColourItem extends CustomItem  {
	...
	String punctuationChars = "(){}[]<>+=',;:\\\n\t|&@$*?/!%#\" ";
  	...
  	public StringColourItem(StringColourItemText stringColourItemTextParam,
  										int screenWidthParam,
  										int fontSizeParam,
  										int indexOfItem,
  										MainForm applicationMainFormParam) {
  	super(null); 
 ...
 * protected void sizeChanged(int w,  int h){
  ...
  private Font setFont(FontStyle fontStyle) {
	...
	public void keyPressed(int keyCode) {
    		int keyTranslate = -5; // center key - translate
    		int keySelectForward = 51; //3 key- move selection back
    		int keySelectBack = 49; // 1 key - move selection forward
  	    //Util.getUtil().log(Integer.toString(keyCode));

		if(keyCode == keyTranslate){
			try {	applicationMainForm.translateToBeTranslatedWordTextField(false);	}
			catch (Throwable t) { Util.getUtil().log(t);	}
		}  	
  ...
  public void selectNextWord(){
	...
	if(currSelectItem == -1){ // select first word
  	...
  	if(isSelected){
  		...
  		itemTextWrap[currSelectItem] = itemText.substring(0, firstSelPos) + selectChar + selectedWord + selectChar +
  												itemText.substring(lastSelPos);
  ...
  private String ClearSelectChar(String s){
   ...
   public void pointerPressed(int x, int y) {
  	/// clear the last selection word
  	if(currSelectItem != -1) itemTextWrap[currSelectItem] = ClearSelectChar(itemTextWrap[currSelectItem]);
  	if(applicationMainForm.indexOfSelectedItem != -1 &&
  		applicationMainForm.indexOfSelectedItem != indexOfTranslationItem)
  		applicationMainForm.setCurrentSelectedItem(-1, null,  false);
  	currSelectItem = -1;
  	selectedWord = null;
  	
  	/// get new selection word
   	...
   	if(selectMode == PredefinedContent.selectionModeAll){
		...
		if(isOverXPoint){
			...
			if(lastPos < nextPos){
				...
				itemTextWrap[itemIndex] = itemTextWrap[itemIndex].substring(0, lastBreakPos) + selectChar + selectedWord + selectChar + 
													itemTextWrap[itemIndex].substring(nextBreakPos);
				isSelected = true;
				currSelectItem = itemIndex;
			}
		}   		
	...
	if(isSelected){
		try {applicationMainForm.translateSelectedWord(removeBreakChar(selectedWord));}
		catch (Throwable t) { Util.getUtil().log(t);	}
	}
  }
